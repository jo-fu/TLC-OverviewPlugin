...
app
.controller('MainCtrl', function($scope, $http, $sce, OverviewAPI, CreateArray, CreateTimeline, DateHandling, DateExporting) {

	// new overview variables
	$scope.doclength = 0;
	$scope.currDocNr = 0;
	...
	// Get documents from Overview
	OverviewAPI.getDocs($scope);

	// PYTHON CALL
	$scope.getTimexes = function(last,data){
		var myData = preprocessing(data)
		//console.log(myData)
		if(myData!=""){
	...
	window.setInterval(function(){ $scope.saveState($scope, "autosave") }, 60000);
})
...
// SERVICES
.service("OverviewAPI" , function(){

	// GET LIST OF DOCS FROM OVERVIEW
	this.getDocs = function($scope){

		var parsedDoc = documents[0]
					.replace(/u&#39;/g, '\"')
					.replace(/&#39;/g, '\"')
					.replace(/None/g, '\"\"');

		var overview_documents = $.parseJSON('['+parsedDoc+']')[0];
		$scope.doclength = overview_documents.length;
		overview_documents.forEach( function(d,i) {
			callSingleDoc(d.id,i)
		})

		// Call single docs
		function callSingleDoc(thisid,i){
			thisID = { 'thisID' : thisid }
			$.ajax({
			    type: "POST",
			    ...
			    contentType: 'application/json;charset=UTF-8',
			    success: function(data){
			    	// Cleaning returned text
			    	var myText = data.result.text
			    		// get rid of line breaks
			    		.replace(/(?:\r\n|\r|\n)/g, '')
			    		// remove whitespace
			    		.replace(/\s\s+/g, ' ');
			    	data.result.text = myText;
			    	if(i==$scope.doclength-1){ var last = true; }
			    	else{ var last = false; }
			    	// Send data to get the timexes
			    	$scope.getTimexes(last,data.result)
			    }
			})
		}
...
app.service('DateHandling', function(){
	...
	this.enableEdit = function($scope, el){
		...
		if(el=="t"){
			...
			// Adjust textareasize
			var boxH = $("#thiscontent").height();
			console.log(boxH)
			$scope.editContent = true;
			setTimeout( function(){	
				if(boxH!=0) $("#displayContent textarea").height(boxH);
			},10)
		}
	...
	this.makeSelection = function($scope, sentNr, d, origin){
		...
		// Connect to Overview's document view - outside of iFrame
 ...
 //      	//return $scope
	...
	this.hideDoc = function(v,tx){
		...
		if($("#button_"+v).hasClass("inactive")){
			$("#button_"+v).removeClass("inactive").html("&#10004;")
		...
		// If document is active - deactivate it
			...
			$("#button_"+v).addClass("inactive").html("&#10008;")
